...
export default function GameDetails() {
  ...
  async function queryGame() {
    try {
      const res = await fetch(`/api/auth/game?title=${encodeURIComponent(title ?? "")}`, {
        method: "GET",
      ...
      <section className="mx-auto grid max-w-6xl gap-8 md:grid-cols-[340px,1fr]">
        ...
        <div className="space-y-5 rounded-2xl border border-[#1ec3ff]/25 bg-white/5 p-6">
          ...
          <section className="space-y-2">
            ...
            <p className="font-semibold text-gray-300">Developed By: {game.main_developer}</p>
            <p className="font-semibold text-gray-300">Published By: {game.publisher}</p>
            <p className="text-gray-300">{game.description}</p>
            <p className="font-semibold text-gray-300">Platforms:</p>
            <ul className="flex flex-row">
              {game.platforms.map((el) => (
                <li key={el} className="text-gray-300 px-1">
                  {el}
                </li>
              ))}
            </ul>
        ...
        {messageState === "" ? (
          <button
            className="md:col-span-2 rounded-lg border border-[#1ec3ff]/40 mt-10 px-3 py-1.5 text-[#a7e9ff] hover:bg-[#1ec3ff]/10"
  ...
  return (
    <div className="flex items-center" aria-label={`Rating: ${value} out of 10`}>
      {[1, 2, 3, 4, 5].map((i) => {
        ...
        const starFill = Math.max(0, Math.min(1, (value - base) / 2));
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 overflow-hidden" style={{ width: `${starFill * 100}%` }} aria-hidden>
